...
def _write_concat_info(file_name, segment_count, out):
    ...
    with open(os.path.join(SEGMENT_DIR, f"{file_name}-concat_info.txt"), "w+") as file:
        for segment_number in range(segment_count):
            f = "file " + f"{file_name}-{segment_number}{SEGMENT_EXTENSION}\n"
            file.write(f)
...
async def _download_worker(
    download_queue: asyncio.Queue, decrypt_pipe_input, client: aiohttp.ClientSession
):
    ...
    while True:
        ...
        decrypt_pipe_input.send((resp_data, key, file_name, segment_number))
...
class Downloader(object):
    ...
    async def run(self):
        ...
        for segment_number, segment in segment_list:
            await download_queue.put(
                (
                    os.path.join(
                        SEGMENT_DIR,
                        f"{self._resume_code}-{segment_number}{SEGMENT_EXTENSION}",
                    ),
                    segment,
                    segment_number,
                )
        ...
        workers = [
            asyncio.create_task(
                _download_worker(download_queue, decrypt_pipe_input, client)
            )
        ...
        _write_concat_info(
            self._resume_code, len(stream.segments), self._output_file_name
        )
